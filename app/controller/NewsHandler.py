...
class ShowNews(BaseHandler):
    def get(self, id):
        news = self.db.get("select news.id, news.title, news.content, news.summary, news.postedat, news.commentnum, usr.name as author, category.name as category, news.doc as doc, news.categoryid from news left join usr on news.author=usr.id left join category on news.categoryid=category.id where news.id=%s;" % id)
        if news:
