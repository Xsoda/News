...
class ShowNews(BaseHandler):
    def get(self, id):
        ...
        if news:
            news['content'] = self.markdown(news['content'])
...
class Preview(BaseHandler):
    ...
    def post(self):
        ...
        self.write(self.markdown(content))
...
class EditNews(BaseHandler):
   ...
   def post(self, id):
       ...
       content = self.get_argument('content', None)
       source = self.get_argument('source', None)
       if self.db.execute_rowcount("update news set categoryid=%s, title=%s, content=%s, source=%s where id=%s;", *(categoryid, title, content, source, id)):
           self.write('done')
       else:
           self.write('undone')
       self.finish()
