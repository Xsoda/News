...
class AddNews(BaseHandler):
    ...
    def post(self):
        ...
        if self.db.execute_rowcount("insert into news(categoryid, title, content, source, author, postedat, commentnum) values(%s, %s, %s, %s, %s, %s, %s)", *(categoryid, title, markdown.Markdown().convert(content), source, self.current_user['id'], time.ctime(), 0)):
            self.write('done')
        else:
            self.write('undone')
